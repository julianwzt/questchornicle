/* ==========================================================================
   SCRIPT.JS - QUEST CHRONICLE (MULTI-ENEMY & SMOOTH ANIMATION)
   ========================================================================== */
...
// Load Player Frame 1 (Karena frame 2 tidak ada di folder, kita hanya load frame 1)
...
async function fetchMapFromServer(mapId) {
    try {
        ...
        activeMap = rows.map(row => row.trim().split(/\s+/).map(Number));
...
const chests = [
    ...
    { x: 28 * TILE_SIZE, y: 3 * TILE_SIZE, opened: false, item: 'sword' },
    { x: 45 * TILE_SIZE, y: 8 * TILE_SIZE, opened: false, item: 'potion' },
    { x: 10 * TILE_SIZE, y: 25 * TILE_SIZE, opened: false, item: 'potion' },
    { x: 35 * TILE_SIZE, y: 35 * TILE_SIZE, opened: false, item: 'sword' },
    { x: 5 * TILE_SIZE, y: 45 * TILE_SIZE, opened: false, item: 'potion' },
    { x: 42 * TILE_SIZE, y: 42 * TILE_SIZE, opened: false, item: 'potion' }
];
...
// Template membuat Musuh
function createEnemy(x, y, maxHp, damage, skin) {
    return {
        x: x, y: y,
        width: 40, height: 40,
        hp: maxHp, maxHp: maxHp,
        damage: damage, speed: 1.2,
        alive: true, attackCooldown: 0,
        state: 'IDLE', patrolDir: { x: 0, y: 0 },
        patrolTimer: 0, lastPlayerX: 0, lastPlayerY: 0,
        skin: skin, 
...
function initEnemies() {
    // Menyebarkan 7 musuh dengan HP tipis (50-80) dan tipe beragam
    enemies = [
        createEnemy(15 * TILE_SIZE, 8 * TILE_SIZE, 50, 10, 'slime'),
        createEnemy(30 * TILE_SIZE, 12 * TILE_SIZE, 80, 15, 'orc'),
        createEnemy(10 * TILE_SIZE, 25 * TILE_SIZE, 40, 5, 'bat'),
        createEnemy(40 * TILE_SIZE, 20 * TILE_SIZE, 50, 10, 'slime'),
        createEnemy(25 * TILE_SIZE, 35 * TILE_SIZE, 80, 15, 'orc'),
        createEnemy(8 * TILE_SIZE, 42 * TILE_SIZE, 40, 5, 'bat'),
        createEnemy(40 * TILE_SIZE, 45 * TILE_SIZE, 50, 10, 'slime')
    ];
}

let player = { 
    x: 3 * TILE_SIZE, y: 3 * TILE_SIZE, speed: 2.5,
    ...
    resName: '', resVal: 0, resMax: 100, isDefending: false,
    ...
    skill2() {
        if (this.job === 'Warrior' && this.resVal >= 20) {
            this.resVal -= 20; this.isDefending = true;
...
function addInventoryItem(name, type) {
    ...
    else inventory.push({ name: name, type: type, count: 1 });
}
...
function renderInventory() {
    ...
    inventory.forEach((item, index) => {
        ...
        itemDiv.className = 'menu-btn'; itemDiv.style.width = '320px'; itemDiv.style.display = 'flex';
        itemDiv.style.justifyContent = 'space-between'; itemDiv.style.alignItems = 'center';
        itemDiv.style.padding = '10px 15px'; itemDiv.style.margin = '5px 0'; itemDiv.style.cursor = 'default';
        ...
        if (item.type === 'potion') {
            ...
            useBtn.innerText = 'USE'; useBtn.style.background = '#2ecc71'; useBtn.style.border = '2px solid #27ae60';
            useBtn.style.color = 'white'; useBtn.style.padding = '6px 12px'; useBtn.style.cursor = 'pointer';
            useBtn.style.fontWeight = 'bold'; useBtn.style.borderRadius = '5px';
...
function usePotion(index) {
    ...
    if (item && item.type === 'potion' && item.count > 0) {
        if (player.hp >= player.maxHp) { alert("Darah pahlawan sudah penuh!"); return; }
...
function spawnProjectile(x, y, dir, damage, type = 'archer') {
    ...
    const textureName = type === 'mage' ? `petir_${projectileDirMap[dir]}_1` : `arrow_${projectileDirMap[dir]}`;
...
function showScreen(id) { 
    ...
    if (targetOverlay) { targetOverlay.classList.add('active'); resetMenuSelection(id); }
...
function gameOver() {
    isGameStarted = false; document.getElementById('hud').style.display = 'none'; showScreen('game-over-menu');
}
...
function stageClear() {
    ...
    setTimeout(() => {
        ...
        initEnemies(); // Musuh respawn
        enemies.forEach(e => { e.maxHp += 20; e.hp = e.maxHp; e.damage += 5; });
...
function backToMainMenu() {
    isGameStarted = false; document.getElementById('hud').style.display = 'none'; showScreen('main-menu');
}

async function startGame(job) {
    isGameStarted = true; stageFinished = false; inventory = [];
    player.setJob(job); player.x = 3 * TILE_SIZE; player.y = 3 * TILE_SIZE;
    
    initEnemies(); // Munculkan banyak musuh
    projectiles = []; chests.forEach(c => c.opened = false);

    document.getElementById('job-val').innerText = job; 
    document.getElementById('hud').style.display = 'block'; updateHUD(); 
...
function saveGameData(slotId) {
    ...
    fetch('GameServlet', { method: 'POST', headers: {'Content-Type': 'application/x-www-form-urlencoded'}, body: payload })
    .then(res => res.text()).then(text => {
        try { JSON.parse(text); alert("Progres Berhasil Disimpan di Slot " + slotId + "!"); showScreen('settings-menu'); }
        catch (e) { alert("SERVER ERROR!"); }
    }).catch(err => alert("Gagal koneksi: " + err));
}

async function loadGameData(slotId) {
    fetch('GameServlet', { method: 'POST', headers: {'Content-Type': 'application/x-www-form-urlencoded'}, body: `action=load&slot_id=${slotId}` })
    .then(res => res.text()).then(async text => {
        try {
            let data = JSON.parse(text);
            if (data.status === 'empty' || !data.player) { alert("Slot " + slotId + " kosong!"); showScreen('main-menu'); return; }
        ...
        } catch (e) { alert("Gagal memproses data Server."); }
    });
}
...
window.addEventListener('keydown', e => {
    ...
    if (e.key === 'i' || e.key === 'I') {
        if (gameState === 'PLAYING') { showScreen('inventory-menu'); renderInventory(); }
    ...
    if (activeOverlay && gameState.includes('MENU')) {
        ...
        if (items.length > 0) {
            ...
            } else if (e.key === 'Enter') { e.preventDefault(); items[index].click(); }
        }
    }
    keys[e.code] = true;
...
function checkCollision(x, y, w, h, target) { return (x < target.x + target.width && x + w > target.x && y < target.y + target.height && y + h > target.y); }
function checkCollisionBox(box1, box2) { return (box1.x < box2.x + box2.width && box1.x + box1.size > box2.x && box1.y < box2.y + box2.height && box1.y + box1.size > box2.y); }

function isSolid(x, y) {
    let col = Math.floor(x / TILE_SIZE), row = Math.floor(y / TILE_SIZE);
    if (activeMap.length === 0) return true; 
    ...
    for (let row = startRow; row < endRow; row++) {
        for (let col = startCol; col < endCol; col++) {
            ...
            if (img && img.complete) {
                ...
                const colors = ['#2ecc71', '#7f8c8d', '#3498db', '#d35400', '#27ae60', '#f1c40f'];
...
function drawChests() {
    chests.forEach(chest => {
        ...
        if (img && img.complete && !chest.opened) { 
            ...
            ctx.fillStyle = '#7f8c8d'; ctx.fillRect(cx + 8, cy + 8, TILE_SIZE - 16, TILE_SIZE - 16);
            ctx.strokeStyle = '#000'; ctx.lineWidth = 2; ctx.strokeRect(cx + 8, cy + 8, TILE_SIZE - 16, TILE_SIZE - 16);
        }
    });
}
...
function updateEnemies() {
    enemies.forEach(enemy => {
        ...
        const dx = playerCenter.x - enemyCenter.x; const dy = playerCenter.y - enemyCenter.y;
        const distance = Math.sqrt(dx * dx + dy * dy);

        switch(enemy.state) {
            case 'IDLE':
                if (distance <= ENEMY_AGGRO_RANGE) {
                    enemy.state = 'CHASE'; enemy.lastPlayerX = player.x; enemy.lastPlayerY = player.y;
                } else {
                    enemy.patrolTimer++;
                    ...
                    if (enemy.patrolDir.x !== 0 || enemy.patrolDir.y !== 0) {
                        ...
                        if (canMoveTo(patrolX, enemy.y, enemy.width, enemy.height)) enemy.x = patrolX;
                        if (canMoveTo(enemy.x, patrolY, enemy.width, enemy.height)) enemy.y = patrolY;
                    }
                }
                break;
            case 'CHASE':
                if (distance > ENEMY_DEAGGRO_RANGE) { enemy.state = 'IDLE'; enemy.patrolTimer = ENEMY_PATROL_INTERVAL; break; }
                if (distance <= ENEMY_ATTACK_RANGE) { enemy.state = 'ATTACK'; break; }
                let chaseDx = (dx / distance) * enemy.speed; let chaseDy = (dy / distance) * enemy.speed;
                let newEnemyX = enemy.x + chaseDx;
                if (canMoveTo(newEnemyX, enemy.y, enemy.width, enemy.height)) enemy.x = newEnemyX;
                else {
                    let slideY = enemy.y + (chaseDy > 0 ? enemy.speed : -enemy.speed);
                    if (canMoveTo(enemy.x, slideY, enemy.width, enemy.height)) enemy.y = slideY;
                }
                let newEnemyY = enemy.y + chaseDy;
                if (canMoveTo(enemy.x, newEnemyY, enemy.width, enemy.height)) enemy.y = newEnemyY;
                else {
                    let slideX = enemy.x + (chaseDx > 0 ? enemy.speed : -enemy.speed);
                    if (canMoveTo(slideX, enemy.y, enemy.width, enemy.height)) enemy.x = slideX;
                }
                enemy.lastPlayerX = player.x; enemy.lastPlayerY = player.y;
                break;
            case 'ATTACK':
                if (distance > ENEMY_ATTACK_RANGE) { enemy.state = 'CHASE'; break; }
                if (enemy.attackCooldown <= 0) {
                    let damage = enemy.damage;
                    if (player.isDefending) damage *= 0.3;
                    player.hp -= damage; showFloatingDamage(player.x, player.y, damage, 'red');
...
function drawEnemies() {
    enemies.forEach(enemy => {
        if (!enemy.alive) return;
        const ex = enemy.x - camera.x; const ey = enemy.y - camera.y;
        if (ex + enemy.width < 0 || ex > canvas.width || ey + enemy.height < 0 || ey > canvas.height) return;

        // Mengambil gambar berdasarkan skin dan animasinya
        ...
        if (img && img.complete && img.naturalWidth > 0) {
            ...
            ctx.ellipse(ex + enemy.width/2, ey + enemy.height/2, enemy.width/2, enemy.height/2, 0, 0, Math.PI * 2); ctx.fill();
        }
        ...
        ctx.strokeStyle = '#000'; ctx.lineWidth = 1; ctx.strokeRect(ex, ey - 12, enemy.width, 8);
    });
}
...
function updateProjectiles() {
    for (let i = projectiles.length - 1; i >= 0; i--) {
        ...
        if (hit) { projectiles.splice(i, 1); updateHUD(); }
    }
}

function gameLoop() {
    if (gameState === 'PLAYING') {
        updatePlayer(); updateProjectiles(); updateEnemies(); updateCamera();
        ...
        projectiles.forEach(p => {
            ...
            if (img && img.complete && img.naturalWidth > 0) {
                ...
                ctx.fillStyle = p.kind === 'mage' ? '#3498db' : '#bdc3c7'; ctx.beginPath();
                ctx.arc(p.x - camera.x, p.y - camera.y, 8, 0, Math.PI * 2); ctx.fill();
            }
        });

        for (let i = floatingTexts.length - 1; i >= 0; i--) {
            let text = floatingTexts[i]; ctx.fillStyle = text.color; ctx.font = 'bold 20px Arial';
        ...
        // FIX KEDAP-KEDIP: Memaksa menggunakan gambar "_1" saja karena gambar "_2" tidak ada di folder
        const textureKey = getPlayerTextureKey(player.job || 'Warrior', player.direction, 1);
        const img = assets.player[textureKey];
        
        if (img && img.complete && img.naturalWidth > 0) {
            // EFEK BOBBING: Menggantikan pergantian frame gambar. Karakter bergerak naik-turun sedikit saat jalan!
