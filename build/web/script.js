...
loadAsset("tiles", "37", "res/tiles/037.png"); // Tile Tangga Dungeon
...
let currentMapName = "world01"; // Menyimpan identitas map saat ini
...
function renderInventory() {
  ...
  if (document.getElementById("inv-atk"))
    document.getElementById("inv-atk").innerText =
      `${player.baseAtk} (DEF: ${player.def})`;
...
// --- LOGIKA SPAWN DI BAWAH TILE 37 ATAU 11 (DUNGEON) ---
async function enterDungeon() {
  if (isTransitioning) return;
  isTransitioning = true;
  gameState = "TRANSITION"; // Berhenti sementara
  ...
  await fetchMapFromServer("dungeon2"); // Memanggil map dungeon2.txt

  // Mencari otomatis posisi tangga di dungeon2.txt
  let spawnX = 240;
  let spawnY = 240;
  for (let r = 0; r < activeMap.length; r++) {
    for (let c = 0; c < activeMap[r].length; c++) {
      if (activeMap[r][c] === 37 || activeMap[r][c] === 11) {
        spawnX = c * TILE_SIZE;
        spawnY = (r + 1) * TILE_SIZE; // Posisi di bawah tangga persis
        break;
      }
    }
  }
  ...
  fetch("GameServlet", {
    ...
    .then((data) => {
      ...
      gameState = "PLAYING";
      requestAnimationFrame(gameLoop); // <-- Memicu kembali gameLoop
    });
}
...
function updatePlayer() {
  ...
  if (
    activeMap[currRow] &&
    (activeMap[currRow][currCol] === 11 || activeMap[currRow][currCol] === 37)
  ) {
    if (currentMapName !== "dungeon2") {
      enterDungeon();
    }
  }
}
